...
When /^(\w+) messages are checked from this point on/ do |who|
  msgs = clients[who].received_messages
  msgs && msgs.clear
end
...
Then /^(\w+) should\s*(not)? receive "([^"]*)"$/ do |who, no, name|
  clients[who].received_messages(name, true).send(no ? :should : :should_not, be_empty)
end
